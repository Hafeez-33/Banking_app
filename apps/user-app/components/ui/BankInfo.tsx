...
const BankInfo = ({ account, appwriteItemId, type }: BankInfoProps) => {
  ...
  return (
    <div
      ...
      className={cn(`gap-[18px] flex p-1 transition-all border bg-blue-50 border-transparent ${colors.bg}`, {
    ...
    >
      ...
      <div className="flex w-full flex-1 flex-col justify-center gap-1">
        <div className="flex flex-1 items-center justify-between gap-2 overflow-hidden">
          ...
          {type === "full" && (
            <p
              className={`text-sm rounded-full px-3 py-1 font-medium text-blue-700 ${colors.subText} ${colors.lightBg}`}
            >
              {account.subtype}
            </p>
          )}
